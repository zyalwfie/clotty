...
if (isset($_POST['save'])) {
    ...
    $sql = mysqli_query($con, "INSERT INTO transaction_t VALUES ('$number','$date','$phone_number','$code_t','$payment')");
    if ($sql) {
...
<head>
    ...
    <title>Transaction Trousers</title>
</head>
...
<body>
    <div class="container-fluid">
        ...
        <div class="container text-center">
            <h1>Checkout your trousers</h1>
            <p class="lead">Below is an form data checkout, make sure before you confirm your choose. Because if it has been confirmed, it cannot<br> be changed again. This some <a href="transaction_t.php">list of transaction.</a> Before make a transaction, <a href="input_customer.php">register</a> your data first.</p>
        </div>
        <div class="container">
            <!-- Form -->
            <form method="post" class="w-50 mx-auto">
                ...
                <div class="input-group mb-3">
                    <span class="input-group-text" id="code_t"><i class="bi bi-bag"></i></span>
                    <select type="text" class="form-select" name="code_t">
                        <?php
                        include("connect.php");
                        $sql = mysqli_query($con, "SELECT * FROM trousers ORDER BY code_t");
                        ?>
                        <?php
                        while ($r = mysqli_fetch_array($sql)) {
                            echo "<option value=$r[code_t]>$r[code_t]</option>";
                        }
                        ?>
                    </select>
                </div>
